...
void QuadContext::add(QuadNode* node, Value* newname) {
	roots->push_back(node);
	if (NULL != newname) {
		// For the normal ones with a result
		QuadNode* someExist = nodeMap->find(newname->var)->second;
		if (someExist != NULL && someExist != node) {
			// An name comes with some new value, remove it from the original value
			someExist->removeSynonym(newname);
		}
		nodeMap->insert(
				std::pair<char*, QuadNode*>(newname->var, node));
		node->addSynonym(newname);
	} else {
